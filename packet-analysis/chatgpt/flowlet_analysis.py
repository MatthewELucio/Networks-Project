...
Analyze flowlet features to understand which are most important for classification.
Generates correlation matrices, feature importance, and distribution comparisons.

Usage: python3 flowlet_analysis.py <features.json> --output analysis_results.json
"""
import argparse
import json
import numpy as np
...
def main(argv=None):
    p = argparse.ArgumentParser(
        description="Analyze flowlet features for LLM classification"
    )
    ...
    p.add_argument(
        ...
        help="directory for output plots (default: analysis_plots)",
    )
    p.add_argument(
        "--test-size",
        type=float,
