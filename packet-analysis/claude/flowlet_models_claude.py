#!/usr/bin/env python3
"""flowlet_models_claude.py

Train classification models to distinguish Claude vs non-LLM flowlets.
Uses MaMPF-inspired approach with Markov models and traditional ML classifiers.

Usage: python3 flowlet_models_claude.py <features.json> --output results_claude.json
"""
import argparse
import json
import numpy as np
...
def filter_claude_only(features: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Filter to only Claude (llm) vs non-LLM flowlets."""
    filtered = []
    for f in features:
        source = f.get("source_file", "")
        traffic_class = f.get("traffic_class", "")

        # Only keep Claude and non-LLM
        if "claude" in source.lower() or traffic_class == "non_llm":
            filtered.append(f)
...
def build_markov_model(sequences: List[List[str]]) -> Dict[str, Any]:
    ...
    end_probs = {state: count / total_sequences for state, count in end_counts.items()}
...
def prepare_training_data(
    ...
    np.ndarray, np.ndarray, List[str], Dict[str, Any], Dict[str, Dict[float, str]]
]:
    ...
    for class_name, flowlets in [("llm", llm_flowlets), ("non_llm", non_llm_flowlets)]:
        ...
        for f in flowlets:
            ...
            if packet_sizes and class_name in block_mappings:
                size_seq = [
                    block_mappings[class_name].get(s, f"BLOCK_{s:.2f}")
...
def train_and_evaluate_models(
    ...
    results["random_forest"] = {
        ...
        "classification_report": classification_report(
            y_test, y_pred_rf, target_names=["non_llm", "llm"], output_dict=True
        ),
    }
    ...
    results["svm"] = {
        ...
        "classification_report": classification_report(
            y_test, y_pred_svm, target_names=["non_llm", "llm"], output_dict=True
        ),
    }
    ...
    results["xgboost"] = {
        ...
        "classification_report": classification_report(
            y_test, y_pred_xgb, target_names=["non_llm", "llm"], output_dict=True
        ),
    }
...
def main(argv=None):
    p = argparse.ArgumentParser(
        description="Train classification models for Claude vs non-LLM flowlets"
    )
    p.add_argument("input", help="JSON file with flowlet features")
    p.add_argument(
        "--output",
        "-o",
        default="model_results_claude.json",
    ...
    features = filter_claude_only(features)
    print(f"Filtered to {len(features)} Claude/non-LLM flowlets")
    ...
    # Print results
    print("\n" + "=" * 60)
    print("RESULTS - CLAUDE vs NON-LLM")
    print("=" * 60)
