...
specifically for Claude-related traffic.
"""
import argparse
import json
import numpy as np
...
def filter_claude_only(features: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Filter to only Claude (sensitive) vs all non-sensitive flowlets."""
    filtered = []
    for f in features:
        source = str(f.get("source_file", "")).lower()
        
        # Determine effective class using hoisted sensitivity label or metadata
        ...
        # Logic: Keep if it is Claude sensitive traffic OR if it is any non-sensitive traffic
        if ("claude" in source and not is_non_sensitive) or is_non_sensitive:
            filtered.append(f)
...
def main(argv=None):
    p = argparse.ArgumentParser(description="Train Sensitive vs Non-Sensitive models for Claude")
    p.add_argument("input")
    p.add_argument("--output", "-o", default="results_claude.json")
    p.add_argument("--test-size", type=float, default=0.2)
    args = p.parse_args(argv)
    
    print(f"Loading features from {args.input}...")
    raw_features = load_flowlet_features(args.input)
    features = filter_claude_only(raw_features)
    print(f"Filtered to {len(features)} Claude/non-sensitive flowlets")
    
    if len(features) == 0:
        print("Error: No flowlets found matching Claude criteria.")
        return
    ...
    print("\n" + "=" * 60)
    print("RESULTS - CLAUDE SENSITIVITY")
    print("=" * 60)
