#!/usr/bin/env python3
"""flowlet_analysis_claude.py

Analyze Claude flowlet features to understand which are most important for classification.
Generates correlation matrices, feature importance, and distribution comparisons.

Usage: python3 flowlet_analysis_claude.py <features.json> --output analysis_results_claude.json
"""
import argparse
import json
import numpy as np
...
def main(argv=None):
    p = argparse.ArgumentParser(
        description="Analyze Claude flowlet features for LLM classification"
    )
    p.add_argument("input", help="JSON file with flowlet features")
    p.add_argument(
        "--output",
        "-o",
        default="analysis_results_claude.json",
        help="output JSON file for analysis results",
    )
    p.add_argument(
        "--output-dir",
        default="analysis_plots_claude",
        help="directory for output plots (default: analysis_plots_claude)",
    )
    p.add_argument(
        "--test-size",
        type=float,
